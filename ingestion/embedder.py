...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        self.model_configs = {
            ...
            "all-MiniLM-L6-v2": {"dimensions": 384, "max_tokens": 512},  # Local sentence-transformers
            "local-model": {"dimensions": 384, "max_tokens": 512}  # Local model fallback
        }
        
        if model not in self.model_configs:
            logger.warning(f"Unknown model {model}, using default config for local models")
            # Default to 384 for unknown local models
            self.config = {"dimensions": 384, "max_tokens": 512}
        else:
